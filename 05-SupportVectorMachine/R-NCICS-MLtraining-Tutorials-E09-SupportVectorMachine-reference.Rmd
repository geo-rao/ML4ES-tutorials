...
```{r linear-modelInfo}
getModelInfo(model = "svmLinear", regex = FALSE)[[1]]
```
...
linCtrl <- trainControl(method="cv", number=5, search="grid")
...
linSVR <- train(SOIL_MOISTURE_20_DAILY ~ T_DAILY_AVG + P_DAILY_CALC + SOLARAD_DAILY + SUR_TEMP_DAILY_AVG + RH_DAILY_AVG,
                data = trainData, method = "svmLinear", 
                preProcess = c("center", "scale"),           ## defining the preProcessing steps for the training process
                tuneGrid = paraGrid,
                trControl = linCtrl)
...
linPredicted <- predict(linSVR, newdata = testData)

### Now, we want to calculate the RMSE, R^2, and mean absolute error (MAE) using 
### postResample() function
linTesting <- postResample(linPredicted, testData$SOIL_MOISTURE_20_DAILY)
...
plot(x=linPredicted, y=testData$SOIL_MOISTURE_20_DAILY,
     xlab = "Predicted soil moisture (20 cm) - Linear SVR",
     ylab = "USCRN soil moisture (20 cm)",
...
### To specify that we want to do 5-fold CV, we need to use the function trainControl() from *caret*.
polyCtrl <- trainControl(method="cv", number=5, search="grid")
...
## target : SOIL_MOISTURE_20_DAILY
## input  : T_DAILY_AVG, P_DAILY_CALC, SOLARAD_DAILY, SUR_TEMP_DAILY_AVG, RH_DAILY_AVG
polySVR <- train(SOIL_MOISTURE_20_DAILY ~ T_DAILY_AVG + P_DAILY_CALC + SOLARAD_DAILY + SUR_TEMP_DAILY_AVG + RH_DAILY_AVG,
                data = trainData, method = "svmPoly", 
                preProcess = c("center", "scale"),           ## defining the preProcessing steps for the training process
                tuneGrid = paraGrid,
                trControl = polyCtrl)
...
### First, we apply the linSVR model to the test data by using function predict()
polyPredicted <- predict(polySVR, newdata = testData)

### Now, we want to calculate the RMSE, R^2, and mean absolute error (MAE) using 
### postResample() function
polyTesting <- postResample(polyPredicted, testData$SOIL_MOISTURE_20_DAILY)
...
### To specify that we want to do 5-fold CV, we need to use the function trainControl() from *caret*.
rbfCtrl <- trainControl(method="cv", number=5, search="grid")
...
## target : SOIL_MOISTURE_20_DAILY
## input  : T_DAILY_AVG, P_DAILY_CALC, SOLARAD_DAILY, SUR_TEMP_DAILY_AVG, RH_DAILY_AVG
rbfSVR <- train(SOIL_MOISTURE_20_DAILY ~ T_DAILY_AVG + P_DAILY_CALC + SOLARAD_DAILY + SUR_TEMP_DAILY_AVG + RH_DAILY_AVG,
...
```{r radial-testing}
### First, we apply the linSVR model to the test data by using function predict()
rbfPredicted <- predict(rbfSVR, newdata = testData)

### Now, we want to calculate the RMSE, R^2, and mean absolute error (MAE) using 
### postResample() function
rbfTesting <- postResample(rbfPredicted, testData$SOIL_MOISTURE_20_DAILY)
...
testingMetric <- data.frame(Linear = linTesting, 
                            Polynomial = polyTesting,
                            Radial = rbfTesting)
